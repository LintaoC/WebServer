...
public:
    MockSession(boost::asio::io_service &io_service, NginxConfig &config)
        : session(io_service, config) {}

    MOCK_METHOD(void, handle_read, (const boost::system::error_code &error, size_t bytes_transferred), ());
...
protected:
    ...
    void SetUp() override {
        mock_session_ = std::make_unique<MockSession>(io_service_, mock_config_);
    }
};
...
TEST_F(SessionTest, HandleCompleteRequestWithProperHeadersAndBody) {
    ...
    // EXPECT_CALL(*mock_session_, handle_write(_))
    //     .Times(1);  // Expect handle_write to be called exactly once

    // Mimic reading data into session
    memcpy(mock_session_->data_, complete_request.data(), complete_request.size());
    mock_session_->start();
    //mock_session_->handle_read(ec, complete_request.size());

    io_service_.run(); // Ensure all async operations complete
}
TEST_F(SessionTest, HandleCompleteRequesty) {
    std::string complete_request = "GET / HTTP/1.1\r\n"
                                   "Host: www.example.com\r\n"
                                   "Content-Type: text/html\r\n"
                                   "Content-Length: 27\r\n"
